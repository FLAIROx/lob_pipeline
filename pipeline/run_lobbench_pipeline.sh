...
for STOCK in $VALID_STOCKS; do
    ...
    BENCH_JOB_ID=$(sbatch --parsable \
        ...
        --partition="${PARTITION}" \
        --export=ALL,REPO_DIR="${REPO_DIR}",PYTHON="${PYTHON}",STOCK="${STOCK}",RUN_NAME="${NAME}",INFER_OUTPUT="${INFER_OUTPUT}",SKIP_HF_COMPARE="${SKIP_HF_COMPARE}" \
